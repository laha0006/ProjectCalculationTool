...
class TaskControllerTest {
    ...
    void createTask() throws Exception {
        mockMvc.perform(post("/task/{projectId}/create", 1)
                        .with(csrf())
                        .param("taskName", "1")
    ...
    void sendCreateSubTaskForm() throws Exception {
        TaskDto taskDto = new TaskDto("name", "description", 1, LocalDateTime.now(), 1, 1, 1, 1);

        when(taskService.getTaskOnId(1))
                .thenReturn(taskDto);
        mockMvc.perform(get("/task/{taskId}/create/subtask/{projectId}", 1, 1))
                .andExpect(status().isOk())
                .andExpect(view().name("task/createTask"));
    }
